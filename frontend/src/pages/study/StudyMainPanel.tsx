  ...
  const fetchMoves = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        render: (options) => (
          <ErrorToast
            description={`Error getting moves to review: ${
    ...
    async (id: string, success: Boolean) => {
      ...
      try {
        ...
        toast({
          ...
          render: (options) => (
            <ErrorToast
              description={`Error sending review data: ${
                e.response?.errors?.[0]?.message ?? "Please try again later"
              }`}
              onClose={options.onClose}
            />
  ...
  return (
    ...
    >
      ...
      <Text fontSize="16px" mb="5px" mt="0.4em" color="whiteText">
        Studying in ChesSRS reviews the moves that you've added for positions.
        This uses a Spaced Repetition algorithm that spaces out your reviews
        automatically, giving you new reviews each day.
      </Text>
      <Text fontSize="16px" mb="5px" mt="0.4em" color="whiteText">
        All you have to do is make the move that you added on the board. When
        you make an incorrect move, the correct move will be shown and
        highlighted.
      </Text>
    </Flex>
  );
